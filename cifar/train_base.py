...
import models
from data import *
from util_adv import wrm, ifgm, ifgm_main, ifgm_branchy, ifgm_max_v2, ifgm_max_v3, ifgm_max_v4, ifgm_k, ifgm_random, ifgm_idx, ifgm_decision, ifgm_decision_random, ifgm_routing, ifgm_ensemble, fgm, fgm_branchy, fgm_max_v2, fgm_k, wrm, wrm_branchy, wrm_max_v2, wrm_k
import torch.nn.functional as F
import torch.nn as nn
import numpy as np
...
def run_training(args):
    ...
    elif args.defend_algo == 'ifgm_max_v4':
        defend_algo = ifgm_max_v4
    elif args.defend_algo == 'ifgm_routing':
        defend_algo = ifgm_routing
    ...
    for i in range(args.start_iter, args.iters):
        ...
        if defend_algo:  # TO DO: do we need multiply 1/7 ?
            ...
            for idx in range(len(output_branch)):
                loss += w[idx] * train_criterion(output_branch[idx], target_var) * 0.5
            output_adv_branch = model(input_adv_var)
            for idx in range(len(output_adv_branch)):
                if args.loss == 'adjust':
                    loss +=  w[idx] * train_criterion(output_adv_branch[idx], target_var) * 0.5
                else:
                    loss += w[idx] * criterion(output_adv_branch[idx], target_var) * 0.5
        else:
            output_branch = model(input_var)
            loss = 0
            for idx in range(len(output_branch)):
                loss += w[idx] * train_criterion(output_branch[idx], target_var)
...
def validate_adv(args, test_loader, model, criterion, attack_algo, T = None, K=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top1_list = []
...
def validate_one_adv_ensemble(args, test_loader, model, criterion, attack_algo, T, K=0, flop_table=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    for i, (input, target) in enumerate(test_loader):
        ...
        for j in range(input.size(0)):
            ...
            if (measure_branch1.data).cpu().numpy()[j] * ( 1 + np.random.uniform(low=-r[0], high=r[0])) < T[0] :
                out = 0
                for k in range(1):
                    out += T[k] * sm(Variable(torch.from_numpy(output_branch[k].data.cpu().numpy()[j].reshape((1,-1))).float().cuda()))
                ...
                out = 0
                for k in range(7):
                    out += sm(Variable(torch.from_numpy(output_branch[k].data.cpu().numpy()[j].reshape((1,-1))).float().cuda()))
                loss = criterion(out, tar_var)
                prec1, = accuracy(out.data, tar, topk=(1,))
                top1.update(prec1, 1)
                losses.update(loss.item(), 1)
    ...
    exit_table = [exit_b1.avg, exit_b2.avg, exit_b3.avg, exit_b4.avg, exit_b5.avg, exit_b6.avg, exit_m.avg]
...
def validate_one(args, test_loader, model, criterion, T, flop_table=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    exit_b1 = AverageMeter()
...
def validate_one_route_adv(args, test_loader, model, criterion, attack_algo, T, K=0, flop_table=None, old_decision=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    exit_b1 = AverageMeter()
    ...
    exit_m = AverageMeter()
    decision = []
    dc_loader = decision_loader(args, old_decision)
    # switch to evaluation mode
    model.eval()
    end = time.time()
    for i, (input, target) in enumerate(test_loader):
        old_decision = torch.from_numpy(np.asarray(dc_loader.__next__()))
        #print(old_decision)
        target = target.squeeze().long().cuda(async=True)
        target_var = Variable(target, volatile=True)
        input_adv = attack_algo(input, None, F.cross_entropy,
...
def decision_loader(args, iterable):
    n =args.batch_size
    l = len(iterable)
